...
Original file is located at
    https://colab.research.google.com/drive/1IccFbL1e-4HDzsPHnLlK5U1P5PKO9sXd
...
LABEL_MAP = {
    "audience_experience":  0,   # ← Replace with your first label
    "thematic_interpretation":  1,   # ← Replace with your second label
    "craft_evaluation":  2,   # ← Replace with your third label (remove if you have 2 labels
}
...
print(f"Correct predictions: {len(correct_idx)} / {len(ft_true_ids)}\n")
...
for i, idx in enumerate(correct_idx[:15]):
    ...
    confidence = ft_probs[idx][ft_pred_ids[idx]]
    print(f"--- #{i+1} ---")
    print(f"Text:      {text[:2000]}{'...' if len(text) > 200 else ''}")
    print(f"True:      {true_label}")
    print(f"Predicted: {pred_label}  (confidence: {confidence:.2f})")
    print()
